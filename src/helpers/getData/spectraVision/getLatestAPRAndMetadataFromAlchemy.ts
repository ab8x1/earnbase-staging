...
export default async function getLatestAPRAndMetadataFromAlchemy(
  ...
  const calls = ['0x06fdde03', '0x95d89b41', '0x313ce567'].map((selector, i) => ({
    method: 'eth_call',
    params: [
      {
        from: null,
  ...
  // * 4626 totalAssets() call (TVL in underlying assets)
  const totalAssetsSelector = '0x01e1d114'; // correct selector for totalAssets()
  calls.push({
    method: 'eth_call',
    params: [
      {
        from: null,
        to: tokenAddress,
        data: totalAssetsSelector, // no extra args
      },
      'latest',
    ],
    id: calls.length + 1,
  ...
  const [name, symbol, decimals, totalAssetsResp] = result.splice(0, 4);
  ...
  return {
    ...
    name: ethers.utils.defaultAbiCoder.decode(['string'], name.result)[0] as string,
    symbol: ethers.utils.defaultAbiCoder.decode(['string'], symbol.result)[0] as string,
    decimals: tokenDecimals,
    data: [
      {
        timestamp,
        tvl: totalAssetsNormalized ? Number(totalAssetsNormalized) : undefined,
        apr: {
          '1d':
            rate1d && rate && computeAPRFromRates(rate1d, rate, timestamp - timestamp1d).toString(),
          '7d':
            rate7d && rate && computeAPRFromRates(rate7d, rate, timestamp - timestamp7d).toString(),
          '30d':
            rate30d &&
            rate &&
            computeAPRFromRates(rate30d, rate, timestamp - timestamp30d).toString(),
        },
      },
    ],
  };
